package com.example.dddstart.category.product;

import com.example.dddstart.category.domain.product.*;
...
public class ProductRepositoryTest {
	...
	void save(){
		...
		SqlRowSet rsProd =
			jdbcTemplate.queryForRowSet("select * from product where product_id = ?", "PRD-01");
		assertThat(rsProd.next()).isTrue();
		assertThat(rsProd.getInt("price")).isEqualTo(9000);

		SqlRowSet rsImg =
			jdbcTemplate.queryForRowSet("select * from image where product_id = ? order by list_idx", "PRD-01");
	...
	void updateImages() {
		...
		product.changeImages(Arrays.asList(
